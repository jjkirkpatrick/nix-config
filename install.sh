...
OPTIONS:
    ...
    --setup-wifi            Configure WiFi secrets during installation
...
EXAMPLES:
    ...
    $0 -m update --setup-wifi          # Update existing configuration with WiFi
...
get_user_input() {
    ...
    # Ask about WiFi setup
    if [[ "$SETUP_WIFI" == false ]]; then
        echo -e "\n${COLORS[CYAN]}WiFi Configuration:${COLORS[NORMAL]}"
        echo "Your configuration includes WiFi settings that require a secrets file."
        if confirm "Would you like to configure WiFi secrets now?"; then
            SETUP_WIFI=true
        fi
    fi

    # Get WiFi password if needed
    if [[ "$SETUP_WIFI" == true ]]; then
        echo -en "${COLORS[CYAN]}Enter WiFi password for VodafoneD3EB53${COLORS[NORMAL]}: "
        read -s WIFI_PASSWORD
        echo
        if [[ -z "$WIFI_PASSWORD" ]]; then
            log "WARNING" "No WiFi password provided. You'll need to configure it manually later."
            SETUP_WIFI=false
        fi
    fi
    ...
    echo "  Setup WiFi: ${COLORS[YELLOW]}$SETUP_WIFI${COLORS[NORMAL]}"
...
setup_wifi_secrets() {
    ...
    log "INFO" "Setting up WiFi secrets..."
    
    local secrets_file="/etc/nixos/wireless-secrets"
    
    if [[ "$DRY_RUN" == true ]]; then
        log "INFO" "[DRY RUN] Would create WiFi secrets file: $secrets_file"
        log "INFO" "[DRY RUN] Would set secure permissions (600) on secrets file"
        return 0
    fi

    # Create the secrets file with proper content
    if ! sudo tee "$secrets_file" > /dev/null << EOF
# WiFi Network Secrets File
# Generated by NixOS Configuration Installer
...
EOF
    then
        log "ERROR" "Failed to create WiFi secrets file"
        return 1
    fi

    # Set secure permissions
    if ! sudo chmod 600 "$secrets_file"; then
        log "ERROR" "Failed to set secure permissions on WiFi secrets file"
        return 1
    fi

    # Set ownership to root
    if ! sudo chown root:root "$secrets_file"; then
        log "ERROR" "Failed to set ownership on WiFi secrets file"
        return 1
    fi

    log "SUCCESS" "WiFi secrets configured at $secrets_file"
    log "INFO" "WiFi secrets file has been secured with 600 permissions"
}
...
post_installation_info() {
    ...
    if [[ "$SETUP_WIFI" == true ]]; then
        log "INFO" "WiFi secrets configured at: /etc/nixos/wireless-secrets"
    else
        log "INFO" "WiFi setup: Use wireless-secrets.example template to configure manually"
    fi
    echo
